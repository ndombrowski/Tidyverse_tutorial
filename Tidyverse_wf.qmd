...
In this tutorial, we work with a published dataset, the palmer penguins data, so there are no requirements other than downloading the two packages.

::: {layout-nrow=1}
![Image taken from: https://allisonhorst.github.io/palmerpenguins/reference/figures/lter_penguins.png](https://allisonhorst.github.io/palmerpenguins/reference/figures/lter_penguins.png)
:::
...
When we use `~` we use what is called a lambda function, or anonymous function, and we use  `.x` to indicate where the variable in `across()` is used. These two elements come from the purr package that is useful if you want to write effective functions with R.
